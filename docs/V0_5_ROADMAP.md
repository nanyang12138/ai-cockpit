...
| - | --------------------- | ----------------------------------------------------------------------- | --------- | ------- | ---------- |
| 1 | `planner-replan`      | graph edge `decision → planner` for `replan_then_retry` — **CONTRACT LOCKED** at `docs/V0_5_ROW_1_PLANNER_REPLAN_CONTRACT.md` | ≤6        | ≤300    | none       |
| 2 | `reviewer-findings`   | reviewer emits `objective_findings`, next coder turn reads it — **CONTRACT LOCKED** at `docs/V0_5_ROW_2_REVIEWER_FINDINGS_CONTRACT.md` | ≤7        | ≤350    | #1 (recommended)        |
...
| 11 | `chat-mode` | `ai-cockpit chat [QUESTION]` invokes cursor's interactive (or one-shot) mode with project config + memory pre-loaded as system prompt. Read-only by default (cursor `--read-only` or git-stash snapshot fallback). Two modes coexist: workflow (existing `run`/`plan`) and chat (this row). — **CONTRACT LOCKED** at `docs/V0_5_ROW_11_CHAT_MODE_CONTRACT.md`; impl-a gate granted 2026-05-18 07:30 UTC | ≤8 combined (split a+b) | ≤450 combined | none (orthogonal to graph rows; uses existing cursor adapter) |
